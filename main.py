...
async def process_checkout_step(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if current_context == "checkout_name":
    ...
    elif current_context == "checkout_payment":
        keyboard = [
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)
        await update.message.reply_text("💳 Please select your **payment method**:", parse_mode='Markdown', reply_markup=reply_markup)
    ...
    elif current_context == "checkout_correction":
        session['current_context'] = "checkout_name"
        session['checkout_data'] = {}
        await update.message.reply_text("🔄 **Starting Fresh Checkout**\n\nPlease enter your **full name**:", parse_mode='Markdown')
...
async def show_checkout_confirmation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    confirmation_text = "📋 **Order Confirmation Summary**\n\n"
    
    confirmation_text += "👤 **Customer Details:**\n"
    ...
    confirmation_text += "🛍️ **Products Ordered:**\n"
    total = 0
    for item in cart.values():
        item_total = item["price"] * item["quantity"]
        total += item_total
    ...
    confirmation_text += "⚠️ **Important:** If any correction is needed, please enter all the details for one time if you mistake.\n\n"
    
    keyboard = [
        [InlineKeyboardButton("✅ Confirm & Continue", callback_data="confirm_details")],
...
async def handle_payment_selection(update: Update, payment_method: str):
    ...
    # TODO: For "Online Payment", integrate with a payment gateway API here.
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if data.startswith("category_"):
            ...
            reply_markup = InlineKeyboardMarkup(keyboard)
            await query.edit_message_text("💳 Please select your **payment method**:", parse_mode='Markdown', reply_markup=reply_markup)
        
        elif data == "make_corrections":
            session = get_user_session(user_id)
            session['current_context'] = "checkout_correction"
            await query.edit_message_text("✏️ **Making Corrections**\n\nYou can now re-enter all your details from the beginning. Please type anything to restart the checkout process with fresh details.", parse_mode='Markdown')
